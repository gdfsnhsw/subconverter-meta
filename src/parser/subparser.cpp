...
string_array ss_ciphers = {"rc4-md5", "aes-128-gcm", "aes-192-gcm", "aes-256-gcm", "aes-128-cfb", "aes-192-cfb",
                           ...
                           "xchacha20-ietf-poly1305", "salsa20", "chacha20", "chacha20-ietf"};
...
void vmessConstruct(Proxy &node, const std::string &group, const std::string &remarks, const std::string &add,
                    ...
                    const std::string &edge, const std::string &tls, const std::string &sni, const std::string &alpn,
...
void hysteriaConstruct(Proxy &node, const std::string &group, const std::string &remarks, const std::string &server,
...
void trojanConstruct(Proxy &node, const std::string &group, const std::string &remarks, const std::string &server,
                     ...
                     const std::string &mode, const std::string &host, const std::string &path, const std::string &flow,
                     const std::string &tls, tribool udp, tribool tfo, tribool scv, tribool tls13) {
...
void explodeNetch(std::string netch, Proxy &node) {
    ...
    switch (hash_(type)) {
        ...
        case "Trojan"_hash:
            ...
            if (group.empty())
                group = TROJAN_DEFAULT_GROUP;
            trojanConstruct(node, group, remark, address, port, password, transprot, mode, host, path, flow, tls, udp,
                            tfo, scv);
            break;
        case "Snell"_hash:
            obfs = GetMember(json, "OBFS");
    ...
    for (uint32_t i = 0; i < yamlnode[section].size(); i++) {
        ...
        switch (hash_(proxytype)) {
            ...
            case "trojan"_hash:
                ...
                trojanConstruct(node, group, ps, server, port, password, net, mode, host, path, flow, tls, udp, tfo,
                                scv);
                break;
            case "snell"_hash:
                group = SNELL_DEFAULT_GROUP;
        ...
        */
        ...
        switch (hash_(configs[0])) {
            ...
            case "trojan"_hash: // surge 4 style trojan proxy
                ...
                trojanConstruct(node, TROJAN_DEFAULT_GROUP, remarks, server, port, password, "", mode, host, "", flow,
                                tls, udp, tfo, scv);
                break;
            case "snell"_hash:
                server = trim(configs[1]);
                ...
                switch (hash_(remarks)) {
                    ...
                    case "trojan"_hash: //quantumult x style trojan link
                        ...
                        if (host.empty() && !isIPv4(server) && !isIPv6(server))
                            host = server;
                        trojanConstruct(node, TROJAN_DEFAULT_GROUP, remarks, server, port, password, "", mode, host, "",
                                        flow, tls, udp, tfo, scv, tls13);
                        break;
        ...
        switch (to_int(configType, 0)) {
            ...
            case 6: //ss/ssr
                ...
                } else //is ssr cipher
                {
                    ...
                    ssrConstruct(node, group, remarks, base64Encode(remarks), server, port, protocol, cipher, obfs,
                                 pass, obfsparam, protoparam);
                }
                break;
            default:
                continue;
